...
describe 'Resque::Plugin finding hooks' do
  ...
  it 'before_perform hooks are found and sorted' do
    assert_equal %w[before_perform before_perform1 before_perform2], Resque::Plugin.before_hooks(SimplePlugin).map(&:to_s)
  end

  it 'after_perform hooks are found and sorted' do
    assert_equal %w[after_perform after_perform1 after_perform2], Resque::Plugin.after_hooks(SimplePlugin).map(&:to_s)
  end

  it 'around_perform hooks are found and sorted' do
    assert_equal %w[around_perform around_perform1 around_perform2], Resque::Plugin.around_hooks(SimplePlugin).map(&:to_s)
  end

  it 'on_failure hooks are found and sorted' do
    assert_equal %w[on_failure on_failure1 on_failure2], Resque::Plugin.failure_hooks(SimplePlugin).map(&:to_s)
  end
